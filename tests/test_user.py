...
def test_unauthorized_request(client, session):
...
def test_with_expired_token(client, session):
...
def test_with_invalid_token(client, session):
...
def test_with_revoked_token(client, auth):
    headers = auth.login(email="admin@test.com")
    auth.logout(headers)
    response = client.delete("api/v1/logout", headers=headers)
